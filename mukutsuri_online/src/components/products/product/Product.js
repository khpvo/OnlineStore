...
function Product({ product, onAddToCart }) {
  ...
  return (
    <div className="card">
      <img className="bd-placeholder-img card-img-top" src={product.image.url} width="100%" height="100%" aria-label="Product: Image cap" />
      <div className="card-content">
        <div className="card-body">
        <h5 className="card-title text-muted">{product.name}</h5>
        <p className="card-text display-5">{product.price.formatted_with_symbol}</p>
